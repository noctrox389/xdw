...
function onCreatePost()
	...
	if string.match(songName,'balloonsa')  == 'balloonsa' then
		enableNewSystem = 0
		textureSplash2 = 'sunky_splash'
	end
	...
	if sickTrack ~= getProperty('sicks') then
		if sus == false then
			if enableNewSystem == 2 and getPropertyFromClass('ClientPrefs','noteSplashes') == true and not sus then
				if textureSplash == 'sunky_splash' then
					splashOffsetX = 180
					splashOffsetY = 200
					splashScaleX = 0.5
					splashScaleY = 0.5
		...
		sickTrack = getProperty('sicks, perfect');
	end
end
...
function onUpdate()
	if textureSplash == 'BloodSplash' then
		splashVelocity = 30
	elseif textureSplash == 'sunky_splash' then
		splashVelocity = 34
	else
		splashVelocity = 24
	end
